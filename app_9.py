import streamlit as st  # KAN LURUS KALAU PAKE 'AS' GINI, SORI COK!
import pandas as pd
import numpy as np
...
st.markdown("""
    ...
    /* SUNTIKAN REVISI DI SINI: Warna putih dan huruf lebih besar */
    [data-testid="stMetricLabel"] > div {
        font-size: 1.1rem !important; /* Huruf judul diperbesar sedikit */
        font-weight: 600 !important;   /* Dibikin agak lebih tegas */
        color: #ffffff !important;    /* Warna diubah jadi putih bersih */
    }
...
def load_and_train_all():
    ...
    # --- Sisanya ke bawah sama persis dengan kode lama lu ---
    ...
    for name, (model, pre) in models.items():
        ...
        results.append({"Model": name, "R^2": score})
        
    return trained_models, df_raw, df, pd.DataFrame(results).sort_values("R^2", ascending=False)
   
models, df_raw, df_clean, df_results = load_and_train_all()
...
with tab1:
    ...
    with col_raw1:
        ...
        st.dataframe(df_missing_raw, use_container_width=True, hide_index=True, height=550)
    ...
    with col3:
        ...
        fig3.subplots_adjust(bottom=0.32, top=0.88, left=0.24, right=0.95)
...
with tab4:
    ...
    input_vals = {}
    feature_columns = df_clean.drop(columns=['harga_idr', 'harga_miliar'], errors='ignore').columns
